                ...
                {userState?.friends?.data?.slice(0,9).map((friend, index)=>{
                    return(
                        <div className='h-auto'>
                            <Link to={`/${friend.userId.username}`}><img key={index} src={friend.userId.profileImage} alt="photos" className="w-full h-full rounded-md hover:brightness-95"></img></Link>
                            <Link to={`/${friend.userId.username}`}><span className='text-sm truncate mt-3 hover:underline'>{friend.userId.firstName} {friend.userId.lastName}</span></Link>
                        </div>
                    )
                })}
            </div>
