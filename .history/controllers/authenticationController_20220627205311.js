...
const { createToken, createRefreshToken } = require("../middlwares/authentication");
    ...
    if (!user) {
      ...
      bcrypt.compare(password, user.password).then((passwordIsTrue) => {
        if (passwordIsTrue) {
          ...
          let refreshToken = createRefreshToken(user.username, user.email, user.userId)

          redisClient.set("userId",user.userId)
          res
